import { Models, ModelsTypes } from './models';
...
const transformMethods: MethodType = transforms.reduce<Partial<MethodType>>(
  (acc, transform) => {
...
const u = transformModel<ModelsTypes, 'users', Transforms>(
  Models['users'],
  'users',
  transformNames
);
const o = transformModel<ModelsTypes, 'orders', Transforms>(
  Models['orders'],
  'orders',
  transformNames
);
const elephants = transformModel<ModelsTypes, 'elephants', Transforms>(
  Models['elephants'],
  'elephants',
  transformNames
);
...
test('join and reshaping', () => {
  ...
  expect(query).toBe(
    '(users, orders) | join(orders.userId = users.id) {username: users.name, ordername: orders.name}'
  );
  expect(params).toHaveLength(0);
});

test('basic filtering', () => {
  const [query, params] = elephants.filter(elephants.id.equals(39)).toQuery();
...
test('basic reshaping with no aliasing', () => {
  const [query, params] = elephants
    .shape({ elephantAge: elephants.age })
    .toQuery();
